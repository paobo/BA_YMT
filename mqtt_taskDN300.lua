...
if fskv.get("bauds") == nil then
    ...
    --fskv.set("uptime", 720) ------- 设置自动上传周期，默认60分钟【单位分钟】
                ...
                if event == "recv" then -- 如果收到下行数据
                    if payload ~= nil then
                        if string.sub(payload, 1, 2) == "D3" and
                            string.sub(payload, 3, 17) == mobile.imei(0) and
                            string.sub(payload, 18, 19) == "C4" then -- 设置自动上传周期 D3   869020066349869   C4   60(分钟)
                        ...
                        if string.sub(payload, 1, 2) == "D3" and
                            string.sub(payload, 3, 17) == mobile.imei(0) and
                            string.sub(payload, 18, 19) == "C6" then --  设置波特率
                            bauds = string.sub(payload, 20, #payload)
                            if tonumber(bauds) == 2400 or tonumber(bauds) == 4800 or
                                tonumber(bauds) == 9600 then
                                fskv.set("bauds", bauds)
                                local ubd = fskv.get("bauds")
                                local re1 = {
                                    DEVTYPE = "M3",
                                    SN = device_id,
                                    FUNCCODE = "C6",
                                ...
                                local REDATA1 = json.encode(re1)
                                mqtt_client:publish(pub_topic, REDATA1)
                            end
                        end
                    end
                end
...
local function proc_get_metersum(strs)
    ...
    tmps2 = tonumber(tmps2*100)   --------DN300特殊表具*100，其他*10
    return tmps2
    -- local k2 = string.sub(strs,36,43) --------获得水表累计原始数据
end
